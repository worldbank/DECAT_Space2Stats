...
@pytest.fixture
def stac_file_path():
    test_file_dir = os.path.dirname('space2stats_api/src/space2stats_ingest/METADATA/tests/test_stack_columns.py')
...
def test_stac_columns_vs_parquet(stac_file_path, parquet_file_path):
    ...
    stac_columns = {col["name"]: col["type"] for col in stac_item["properties"]["table:columns"]}
    ...
    assert len(parquet_columns) == len(stac_columns), f"Mismatch in column count: Parquet ({len(parquet_columns)}) vs STAC ({len(stac_columns)})"

    # Assert that column names and types match
    for column_name, column_type in stac_columns.items():
        assert column_name in parquet_columns, f"Column {column_name} is missing in the Parquet file"
        assert parquet_columns[column_name] == column_type, f"Mismatch in column type for {column_name}: Parquet ({parquet_columns[column_name]}) vs STAC ({column_type})"
